...
public class ItemService extends EntityService {
	...
	public static ItemBalance getItemBalance(int itemId, int inventoryId){
		...
		connection = JDBCUtil.getCon();
		try{
			String query = "select sum(ITEM_QTY), item_name, CODE, MIN_LIMIT, MAX_LIMIT, PRICE from item_movment where item_id = ? and inventory_id = ? group by item_name, CODE, MIN_LIMIT, MAX_LIMIT, PRICE";
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setInt(1, itemId);
			preparedStatement.setInt(2, inventoryId);
			resultSet = preparedStatement.executeQuery();
			while(resultSet.next()){
				...
				itemBalance.setItemPrice(resultSet.getInt(6));
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static ItemBalance getItemFromCode(String code){
		...
		connection = JDBCUtil.getCon();
		try{
			String query = "select sum(ITEM_QTY), item_name, MIN_LIMIT, MAX_LIMIT, PRICE, item_id from item_movment where CODE = ? group by item_name, MIN_LIMIT, MAX_LIMIT, PRICE, item_id";
			preparedStatement = connection.prepareStatement(query);
			...
			while(resultSet.next()){
				...
				itemBalance.setItemPrice(resultSet.getInt(5));
				itemBalance.setItemId(resultSet.getInt(6));
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static List<ItemBalance> getItemQuantities(){
		...
		try{
			String query = "SELECT A.* FROM "
				+ " ( SELECT INVENTORY_ID , INVENTORY_NAME , ITEM_ID , MAINGROUP, sub_name , item_name ,HOME, CODE, PRICE, round(SUM(ITEM_QTY), 0) TOTAL_QTY "
				+ " FROM item_movment"
				+ " GROUP BY INVENTORY_ID , INVENTORY_NAME , ITEM_ID , MAINGROUP, sub_name , item_name ,HOME, CODE, PRICE"
				+ ")A ";
			preparedStatement = connection.prepareStatement(query);
			resultSet = preparedStatement.executeQuery();
			while (resultSet.next()) {
				...
				itemBalance.setItemPrice(resultSet.getDouble(9));
				itemBalance.setItemQty(resultSet.getInt(10));
